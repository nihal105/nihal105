# Hi there , I'm Muhammed Nihal

### Data Analyst | Machine Learning Enthusiast | AI Developer

I am passionate about transforming data into actionable insights and building intelligent solutions using Machine Learning, AI, and Data Analytics.

---

##  About Me

*  Data Analytics & Business Intelligence
*  Machine Learning & AI Applications
*  Python Developer
*  Financial Data Analysis
*  Currently learning Deep Learning and MLOps

---

##  Tech Stack

### Languages

Python • SQL

### Data Analysis

Pandas • NumPy • Excel

### Machine Learning

Scikit-Learn • XGBoost

### Visualization

Power BI • Tableau • Matplotlib

### Tools

Git • GitHub • Jupyter Notebook

---

##  Featured Projects

###  StockVision AI

Machine Learning-powered stock market prediction and technical analysis platform.

### AI Based Student Placement Prediction System

Predicts placement outcomes using machine learning models.

###  TATA Online Retail Analysis

Performed exploratory data analysis on retail transaction data to identify customer purchasing patterns, top-selling products, revenue drivers, and actionable business insights using Python and data visualization techniques.

###  Global Superstore Sales Analysis

Business intelligence dashboard and sales analytics project.
###  COVID-19 Global Analysis

Data exploration and visualization of pandemic trends.

---

##  Connect With Me

* LinkedIn:www.linkedin.com/in/muhammed-nihal105
* Email: muhammednihal5105@gmail.com

---

Turning data into decisions through analytics and AI.


<!--
**nihal105/nihal105** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
